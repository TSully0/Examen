...
import type { Medicamento, Cliente, NuevoCliente, Venta } from "../dominio";
...
let semillasData: any = null;

async function init() {
  if (estado) return;
  const res = await fetch(RUTA_SEMILLAS_JSON);
  if (!res.ok) throw new Error("No se pudo cargar semillas.json");
...
export const fuenteJson: FuenteDatos = {
  ...
  crearCliente: async (datosCliente: NuevoCliente) => {
    ...
    if (estado!.clientes.some((c) => c.cedula === (datosCliente as any).cedula)) {
      return Promise.reject(new Error("Cédula ya registrada"));
    }
    const nextId = Math.max(0, ...estado!.clientes.map((c) => c.id)) + 1;
    const nuevo: Cliente = { id: nextId, ...(datosCliente as any) } as Cliente;
  ...
  crearVenta: async (datosVenta: any) => {
    await init();
  ...
  anularVenta: async (id: number) => {
    ...
    if (venta.estado !== (semillasData?.estadoInicial ?? "PENDIENTE")) return Promise.reject(new Error("Sólo se puede anular una venta en estado inicial"));
